  ...
  <div class="player-card" :class="cardClasses" @click="handleClick">
    ...
    <div class="player-header">
      <div class="team-badges">
        <template v-if="Array.isArray(matchups[team.code])">
          <span
            v-for="matchup in matchups[team.code]"
            :key="matchup.code"
            class="team-badge"
            :style="{
        ...
        <span
          ...
          :style="{
            backgroundColor: $func_global.difficultyMap(
              matchups[team.code]?.difficulty,
            ),
          }"
        >
          vs {{ matchups[team.code]?.code }}
        </span>
      </div>
    ...
    <div class="player-image-container">
      ...
      <div v-if="isOwnedByUser" class="owned-overlay">OWNED</div>
...
export default {
  ...
  computed: {
    isActive() {
      return this.teamsPlayingNextFixture.includes(this.team.code);
    },
    isOwnedByUser() {
      return this.userTeam.includes(this.team.esportsTeamId);
    },
    isOwnedByOther() {
      return Object.keys(this.userTeamsPicked).includes(
        this.team.esportsTeamId,
      );
    },
    isSelectable() {
      return (
        this.isActive && this.selectedRole === "team" && !this.isOwnedByOther
      );
    },
    cardClasses() {
      return {
